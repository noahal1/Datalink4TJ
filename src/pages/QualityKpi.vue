...
const kpiDescriptions = [
  ...
  'PPM',
  'Supplier Defects',
  'Cost of Quality(failure cost only)',
  ...
  'Supplier RDR%',
  'Cost of Scrap',
  'Magna FTT - Quality Performance (New)'
]

// 区域列表
const areas = ['TJC', 'TJM', '汇总']
...
const shouldShowRemark = (item) => {
  const isSupplierRDR = item.description === 'Supplier RDR' || item.description === 'Supplier RDR%'
  if (isSupplierRDR) {
    return false
  }

  // Magna FTT低于目标值时显示备注
  const isMagnaFTT = item.description === 'Magna FTT - Quality Performance (New)'
  if (isMagnaFTT) {
...
const loadData = async () => {
  ...
  try {
    ...
    // 获取KPI数据
    const response = await get('/qa/kpi/', {
      params: {
    ...
    if (response.data && response.data.length > 0) {
      ...
      // 如果没有数据，尝试获取目标值
      try {
        const targetResponse = await get('/qa/kpi/targets/', {
...
const openTargetDialog = async () => {
  try {
    // 获取当前目标值
    const response = await get('/qa/kpi/targets/', {
      params: { year: selectedYear.value }
    })
...
const saveTargets = async () => {
  ...
  try {
    ...
    await put('/qa/kpi/targets/', payload)
...
.quality-kpi-table :deep(.v-data-table__wrapper) {
  border-radius: 16px;
}
...
.quality-kpi-table :deep(tbody tr) {
