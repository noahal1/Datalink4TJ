<template>
  <v-card class="role-list-card" elevation="2">
    <v-card-title class="d-flex align-center bg-primary text-white">
      ...
      <v-text-field
        ...
        clearable
      ></v-text-field>
      <v-btn color="white" variant="outlined" class="ml-2" @click="$emit('add-role')">
    ...
    <v-card-subtitle class="d-flex align-center py-2">
      <v-chip size="small" color="info" variant="outlined" class="mr-2">
      ...
      <v-chip size="small" color="warning" variant="outlined">
        <v-icon size="small" class="mr-1">mdi-account-alert</v-icon>
    ...
    >
      ...
      <template v-slot:item.name="{ item }">
        <div class="d-flex align-center">
          <v-avatar size="32" :color="getRoleColor(item)" class="mr-3">
      ...
      <template v-slot:item.permissions_count="{ item }">
        <div class="d-flex align-center">
          ...
          <v-tooltip v-if="item.permissions?.length > 0" location="bottom">
            <template #activator="{ props }">
              <v-icon v-bind="props" size="small" color="info">mdi-information</v-icon>
            </template>
            <div class="pa-2">
              <div class="text-subtitle-2 mb-1">权限列表:</div>
              <div v-for="permission in item.permissions?.slice(0, 5)" :key="permission.id" class="text-caption">
                • {{ permission.module }}.{{ permission.level }}
              </div>
              <div v-if="item.permissions?.length > 5" class="text-caption text-grey">
      ...
      <template v-slot:item.actions="{ item }">
        <v-btn-group density="compact" variant="outlined">
          ...
          >
            ...
            <v-tooltip activator="parent" location="bottom">编辑角色</v-tooltip>
          </v-btn>
          <v-btn
            size="small"
            color="info"
          ...
          >
            ...
            <v-tooltip activator="parent" location="bottom">管理路由权限</v-tooltip>
          </v-btn>
          <v-btn
            size="small"
            color="error"
