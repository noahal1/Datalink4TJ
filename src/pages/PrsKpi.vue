...
const loadData = async () => {
  ...
  try {
    ...
    // 获取KPI数据
    const response = await get('/prs/kpi/', {
      params: {
    ...
    if (response.data && response.data.length > 0) {
      ...
      // 如果没有数据，尝试获取目标值
      try {
        const targetResponse = await get('/prs/kpi/targets/', {
...
const saveData = async () => {
  ...
  try {
    ...
    await put('/prs/kpi/', payload)

    Message.success('PRS冲压部门KPI数据保存成功')
    isDataChanged.value = false
...
const openTargetDialog = async () => {
  try {
    // 获取当前目标值
    const response = await get('/prs/kpi/targets/', {
      params: { year: selectedYear.value }
    })
...
const saveTargets = async () => {
  ...
  try {
    ...
    await put('/prs/kpi/targets/', payload)
