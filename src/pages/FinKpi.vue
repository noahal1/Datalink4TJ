...
const initializeKpiData = () => {
  ...
  // 财务KPI项目
  const kpiItems = [
    { description: 'AP/AR Spread', areas: ['新厂', '老厂', '汇总'] },
  ]
...
const loadData = async () => {
  ...
  try {
    ...
    // 获取KPI数据
    const response = await get('/fin/kpi/', {
      params: {
    ...
    if (response.data && response.data.length > 0) {
      ...
      // 如果没有数据，尝试获取目标值
      try {
        const targetResponse = await get('/fin/kpi/targets/', {
...
const saveData = async () => {
  ...
  try {
    ...
    await put('/fin/kpi/', payload)

    Message.success('财务KPI数据保存成功')
    isDataChanged.value = false
    showChangeAlert.value = false
...
const openTargetDialog = async () => {
  try {
    // 获取当前目标值
    const response = await get('/fin/kpi/targets/', {
      params: { year: selectedYear.value }
    })
    ...
    if (response.data && response.data.length > 0) {
      ...
      const kpiItems = [
        { description: 'AP/AR Spread', areas: ['新厂', '老厂', '汇总'] },
        { description: 'hot forming', areas: ['汇总'] },
        { description: 'laser cutting', areas: ['汇总'] }
      ]
...
const saveTargets = async () => {
  ...
  try {
    ...
    await put('/fin/kpi/targets/', payload)
