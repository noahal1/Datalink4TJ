  ...
  <unified-page-template 
    ...
    <div class="scrollable-table-container">
      <unified-data-table
        ...
        :loading="loading"
        density="comfortable"
        class="production-kpi-table production-data-table frozen-header-table"
        hide-default-footer=""
        :items-per-page="-1"
        hover
        :fixed-header="true"
...
const kpiDescriptions = [
  ...
  'Divisional General Purpose Equipment Overall (New)',
  'Division Overall OEE',
  'Labour Efficiency',
  'Divisional Theoretical Manpower Ratio (New)'
]
...
const loadData = async () => {
  ...
  try {
    ...
    // 获取KPI数据
    const response = await get('/production/kpi/', {
      params: {
    ...
    if (response.data && response.data.length > 0) {
      ...
      // 如果没有数据，尝试获取目标值
      try {
        const targetResponse = await get('/production/kpi/targets/', {
    ...
    console.error('获取生产KPI数据失败:', error)
...
const saveData = async () => {
  ...
  try {
    ...
    const result = await put('/production/kpi/', payload)
...
const openTargetDialog = async () => {
  try {
    // 获取当前目标值
    const response = await get('/production/kpi/targets/', {
      params: { year: selectedYear.value }
    })
...
const saveTargets = async () => {
  ...
  try {
    ...
    await put('/production/kpi/targets/', payload)
...
/* 生产KPI表格专用悬停样式 - 避免闪烁 */
.production-data-table :deep(.v-data-table__tr:hover) {
  background: rgba(34, 197, 94, 0.04) !important;
  transition: background-color 0.15s ease !important;
}

.production-data-table :deep(.v-data-table tbody tr:hover) {
  background: rgba(34, 197, 94, 0.04) !important;
  transition: background-color 0.15s ease !important;
}

/* 禁用其他可能的悬停效果 */
.production-data-table:hover {
